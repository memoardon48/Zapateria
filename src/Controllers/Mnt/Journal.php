    ...
    private function validatePostData()
    {
        ...
        if (isset($_POST["journal_date"])) {
            if (\Utilities\Validators::IsEmpty($_POST["journal_date"])) {
                ...
                $this->viewData["general_errors"][] = "El date no puede ir vacío!";
            }
            $objDate = \DateTime::createFromFormat('Y-m-d', $_POST["journal_date"]);
            if (!($objDate && $objDate->format('Y-m-d') == $_POST["journal_date"])) {
                $this->viewData["has_errors"] = true;
                $this->viewData["general_errors"][] = "Date invalid value!";
            }
        } else {
            throw new Exception("journal_date not present in form");
        }
        if (isset($_POST["journal_type"])) {
            if (!in_array($_POST["journal_type"], array("CREDIT", "DEBIT"))) {
        ...
        if (isset($_POST["journal_description"])) {
            if (\Utilities\Validators::IsEmpty($_POST["journal_description"])) {
                $this->viewData["has_errors"] = true;
                $this->viewData["general_errors"][] = "El journal description no puede ir vacío!";
            }
        } else {
            throw new Exception("journal_description not present in form");
        }
        if (isset($_POST["journal_amount"])) {
            if (!(is_numeric($_POST["journal_amount"]))) {
                $this->viewData["has_errors"] = true;
                $this->viewData["general_errors"][] = "El journal amount debe ser un número decimal!";
            }
            if (floatval($_POST["journal_amount"]) <= 0) {
                $this->viewData["has_errors"] = true;
                $this->viewData["general_errors"][] = "El journal amount debe ser mayor de 0!";
            }
        } else {
